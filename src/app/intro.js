const duction = {
    all: [
      {
        id: "1",
        image:"/Vachan.jpeg",
        name: "Vachan",
        quote: "\" We are hosting some great events , which will make your day memorable, so please join if you like it \"",

       
        insta:"https://www.instagram.com/vardaanwithnodhan/"
      },
    
      {
        id: "2",
        image:"https://scontent.cdninstagram.com/v/t51.2885-19/258731026_459779118901767_1434310398993580285_n.jpg?stp=dst-jpg_s150x150&_nc_ht=scontent.cdninstagram.com&_nc_cat=103&_nc_ohc=EuN9XHtBwJkAX_bmxqT&edm=APs17CUBAAAA&ccb=7-5&oh=00_AfAjaUnzrNfiTyGJEJxtpmrbIX8aFgjOri6KDG_GW08GXA&oe=660215D8&_nc_sid=10d13b",
        name: "Aalap",
        quote: "\" Aalap is a singers club which is going to conduct events on the day of the event. So we will hope you join!\"",
        insta:"https://www.instagram.com/aalapofficial/"
      },
    
      {
        id: "3",
        image:"https://scontent.cdninstagram.com/v/t51.2885-19/412046551_359182106802041_6323071453295358439_n.jpg?stp=dst-jpg_s150x150&_nc_ht=scontent.cdninstagram.com&_nc_cat=105&_nc_ohc=67MBY7Le4D0AX-9bOGD&edm=APs17CUBAAAA&ccb=7-5&oh=00_AfA-3gaThnqQx28ZJ02QPPn6jzdbk9Szav86M_CJuizSbQ&oe=660131FF&_nc_sid=10d13b",
        name: "Mudra",
        quote: "\" Mudra is a Dancers club which is going to conduct events on the day of the event. So we will hope you join! \"",
        insta:"https://www.instagram.com/mudraofficial/"
      },

      {
        id: "4",
        image:"https://scontent.cdninstagram.com/v/t51.2885-19/231585681_831325504186531_5797848488481493219_n.jpg?stp=dst-jpg_s150x150&_nc_ht=scontent.cdninstagram.com&_nc_cat=107&_nc_ohc=r50hr0lRy0gAX_CDIzC&edm=APs17CUBAAAA&ccb=7-5&oh=00_AfA4GA-z6GF315n4tRp70swD2VAQaxMzoJ7TZJ5uGCbAJw&oe=660195A8&_nc_sid=10d13b",
        name: "Aakarshan",
        quote: "\"Aakarshan is arta and dance club which is going to conduct events on the day of the event. So we will hope you join! \" ",
        insta:"https://www.instagram.com/aakarshanofficial/"
      },
      {
        id: "5",
        image:"https://scontent.cdninstagram.com/v/t51.2885-19/324910135_5956884101001489_5671979558690848789_n.jpg?stp=dst-jpg_s150x150&_nc_ht=scontent.cdninstagram.com&_nc_cat=109&_nc_ohc=5TnSGkmWrwEAX8oaiNB&edm=APs17CUBAAAA&ccb=7-5&oh=00_AfC0ZObtYShTiApjzth7Rlg0VkGzy8ShonepaKD7kLWF7g&oe=66027CA9&_nc_sid=10d13b",
        name: "Riti",
        quote: "\" Riti is a Fashion club which is going to conduct events on the day of the event. So we will hope you join! \"",
        linkedin: "https://www.linkedin.com/in/dheeraj-koka-692a81291/",
        insta:"https://www.instagram.com/riti._.official/"
      },
      {
        id: "6",
        image:"https://scontent.cdninstagram.com/v/t51.2885-19/229604920_1493363987688976_8376050562174167657_n.jpg?stp=dst-jpg_s150x150&_nc_ht=scontent.cdninstagram.com&_nc_cat=104&_nc_ohc=PBhwDa9zWawAX8cT0zo&edm=APs17CUBAAAA&ccb=7-5&oh=00_AfCICW9rj0KwiRepsp9o7DgkFF3XCn2y5tO3-drfzyizCg&oe=6601A8A3&_nc_sid=10d13b",
        name: "Recurse",
        quote: "\" Recurse is the coding club which is going to conduct events on the day of the event. So we will hope you join! \"",
        insta:"https://www.instagram.com/recurse.official/"
      },
      {
        id: "7",
        image:"https://scontent.cdninstagram.com/v/t51.2885-19/118773909_956372321530516_4299631276751353126_n.jpg?stp=dst-jpg_s150x150&_nc_ht=scontent.cdninstagram.com&_nc_cat=104&_nc_ohc=OV7jQiRXId0AX-aql-v&edm=APs17CUBAAAA&ccb=7-5&oh=00_AfALTIH2O-KB8eOZVtYwKzrb3OZMgyx80K_16pXG-Z_xag&oe=66015579&_nc_sid=10d13b",
        name: "Kreeda",
        quote: "\" Kreeda is the sports club which is going to conduct events on the day of the event. So we will hope you join! \"",
        insta:"https://www.instagram.com/kreedaofficial/"
      }
    
    ]

}

export default duction;